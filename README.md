...
# 完全信任本地缓存的路径规则(正则):命中时只要本地文件存在就直接使用,
# 不发起条件请求验证(连ETag校验的网络往返都省掉)。
# 代价:如果服务器更新了这类资源,不会自动感知,会一直用旧版本,直到手动清空对应的本地缓存文件。
# 默认对图片(img)和音效(sound)生效,这类资源更新频率低,清空浏览器缓存后数量庞大,
# 逐个发条件请求的网络往返延迟是首次加载缓慢的主因。
TRUST_LOCAL_PATTERN = re.compile(r"/assets/(img|sound)/")
```

### 完全信任本地缓存(跳过验证)

清空浏览器缓存后重新打开游戏时,数百上千个图片/音效资源如果逐个发起条件请求确认"有没有变化",网络往返延迟会让首次加载变得非常慢。`TRUST_LOCAL_PATTERN` 匹配到的路径,只要本地已有缓存文件就直接使用,完全不联网确认,`cache_log.txt` 中记为 `[直接命中,未验证]`。

如果发现某个资源明显没有更新到最新版本,可以手动删除 `cache` 目录下对应的文件(连同同名的 `.ext` 元数据文件),下次请求会重新走完整下载流程。
...
1. 启动代理,有两种方式:
   ...
   **方式二:双击 `start_proxy.bat`(推荐日常使用)**
   ...
   该脚本会以隐藏窗口的方式在后台启动代理,不会弹出黑框命令行窗口。隐藏模式下 mitmproxy 自身的连接日志不会保留(不写入文件、也无窗口可看);你的缓存命中/更新记录仍会正常写入 `cache_log.txt`,用于确认工作状态。

   脚本启动时会自动检测并结束已占用该端口的旧代理进程(如果有),避免上一次没退干净导致端口冲突启动失败。
...
4. 重启浏览器,正常游玩游戏。cache_log.txt(位于脚本所在目录)会打印以下几种状态,便于确认工作情况:
   ...
   - `[直接命中,未验证]`:命中 `TRUST_LOCAL_PATTERN` 规则(默认 img/sound),本地有文件直接使用,未发起任何网络请求验证
